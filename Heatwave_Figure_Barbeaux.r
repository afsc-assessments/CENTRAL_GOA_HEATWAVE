...
dfa<-dfa+geom_hline(yintercept=5,color="black",linetype=2)+geom_line(data=hobday[MONTH%in%c(10:12)&YEAR==2021],size=1.25)
dfa<-dfa+geom_line(data=hobday[MONTH%in%c(10:12)&YEAR%in%c(2017,2021)],aes(color=factor(YEAR)),size=1.25)#+geom_line(data=hobday[MONTH%in%c(10:12)&YEAR>2014],aes(color=factor(YEAR)),size=1)
dfa<-dfa+labs(y=expression('Sea surface temperature ('~degree*C~")"),x="Day of the year",color='Year', title="October through December 1981-2021")
...
dsp<-dsp+geom_hline(yintercept=5,color="black",linetype=3)+geom_line(data=hobday[MONTH%in%c(1:5)&YEAR==2022],size=1.25)
